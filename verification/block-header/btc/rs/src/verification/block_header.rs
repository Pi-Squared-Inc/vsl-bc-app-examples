...
use generation_block_header_btc::generation::block_header::EncodableBitcoinHeader;
use sha2::{Digest, Sha256};

#[derive(Debug, thiserror::Error)]
pub enum VerifyClaimError {
...
/// # Arguments
///
/// * `claim` - A reference to the `Claim` instance to be validated.
...
/// * `Err(VerifyBlockError)` if the claim is invalid or if an error occurs during verification.
pub fn verify(
    hex_encoded_claim: &String,
    public_key: &str,
    context: &ClaimVerificationContext,
) -> Result<(), VerifyClaimError> {
    let claim_bytes = hex::decode(hex_encoded_claim)
        .map_err(|e| VerifyClaimError::SerializeBlockHeaderError(e.to_string()))?;

    let decoded_encodable_claim = <EncodableClaim as SolValue>::abi_decode(&claim_bytes)
        .map_err(|e| VerifyClaimError::DecodeClaimError(e.to_string()))?;

    let decoded_claim: Claim<EncodableBitcoinHeader> = decoded_encodable_claim
        .try_into()
        .map_err(|e: &'static str| VerifyClaimError::SerializeBlockHeaderError(e.to_string()))?;
    ...
    verify_message(public_key, message_bytes, &context.signature)
        .map_err(|e| VerifyClaimError::VerifyBlockError(e.to_string()))?;
