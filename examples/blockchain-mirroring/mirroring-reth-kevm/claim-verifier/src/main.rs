...
use base::vsl_utils::{
    ...
    vsl_types::{
        IdentifiableClaim, IntoSigned as _, SettleClaimMessage,
        private_key_to_signer,
    },
};
use dotenv::dotenv;
use env_logger::Env;
...
use reqwest::Client;
use serde_json::json;
use verification_block_processing_evm::verification::block_processing::verify;
                ...
                for submitted_claim in submitted_claims {
                    ...
                    match verify(&parsed_claim, &parsed_context) {
                        Ok(_) => {
                            ...
                            let signed_verified_claim =
                                verified_claim.into_signed(&verifier_signer).unwrap();
