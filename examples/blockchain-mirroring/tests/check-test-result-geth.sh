  ...
  if [ $curl_exit_status -ne 0 ]; then
    ...
    if [ $jq_exit_status -ne 0 ] || ! [[ "$block_mirroring_records_count" =~ ^[0-9]+$ ]]; then
      ...
      if [ $jq_extract_exit_status -ne 0 ]; then
        echo "Error: Failed to extract the first validation record using jq."
      else
        # Check if .claim_details.MirroringGeth exists in the first claim
        has_mirroring_geth=$(echo "$first_block_mirroring_record" | jq 'has("claim_details") and (.claim_details | has("MirroringGeth"))')

        if [ "$has_mirroring_geth" != "true" ]; then
          echo "First block mirroring record does not contain 'claim_details.MirroringGeth'. Skipping this attempt."
        else
          # Check if .claim_details.MirroringGeth.error is null
          mirroring_geth_error=$(echo "$first_block_mirroring_record" | jq '.claim_details.MirroringGeth.error')

          if [ "$mirroring_geth_error" == "null" ]; then
            echo "Found the valid block mirroring record."
            exit 0
          else
            # Error condition: MirroringGeth.error is not null
            echo "Error: Found a block mirroring record, but 'geth.error' is not null."
      ...
      # Validation record count is 0
      echo "Validation records count is 0."
    fi
  fi
