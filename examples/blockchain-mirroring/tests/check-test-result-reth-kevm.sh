  ...
  if [ $curl_exit_status -ne 0 ]; then
    ...
    if [ $jq_exit_status -ne 0 ] || ! [[ "$block_mirroring_records_count" =~ ^[0-9]+$ ]]; then
      ...
      if [ $jq_extract_exit_status -ne 0 ]; then
        ...
        # Check if .claim_details.MirroringRethKEVM exists in the first claim
        has_reth=$(echo "$first_block_mirroring_record" | jq 'has("claim_details") and (.claim_details | has("MirroringRethKEVM"))')

        if [ "$has_reth" != "true" ]; then
          echo "First block mirroring record does not contain 'claim_details.MirroringRethKEVM'. Skipping this attempt."
        else
          # Check if .claim_details.MirroringRethKEVM.error is null
          reth_error=$(echo "$first_block_mirroring_record" | jq '.claim_details.MirroringRethKEVM.error')

          if [ "$reth_error" == "null" ]; then
            echo "Found the valid block mirroring record."
            exit 0
          else
            # Error condition: MirroringRethKEVM.error is not null
            echo "Error: Found a block mirroring record, but 'MirroringRethKEVM.error' is not null."
