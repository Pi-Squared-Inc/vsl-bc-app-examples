...
import BitcoinMirroringRecordRow from "./BitcoinMirroringRecordRow";
...
interface BitcoinMirroringTableProps {
...
export default function BitcoinMirroringTable({
  claims,
  isLoading,
  isRefreshing = false,
  isPaging = false,
  ...
  const skeletonRows = useMemo(() => {
    return Array.from({ length: SKELETON_ROWS }).map((_, index) => (
      <TableRow key={`skeleton-${index}`} className="animate-pulse h-[53px] [&_td]:border-r" style={{ animationDelay: `${index * 50}ms` }}>
  ...
  return (
    <section aria-label="Claims data" className="claims-table-container">
        ...
        <Table className="table-fixed border w-full min-w-[920px]">
          <TableHeader className="border">
            <TableRow className="hover:bg-transparent [&_th]:border-r">
              <TableHead rowSpan={2} className="px-4 text-pi2-accent-white font-semibold text-base whitespace-nowrap">
                Block Number
              </TableHead>
              <ValidationRecordsTableHeaderCell>{`Claim`}</ValidationRecordsTableHeaderCell>
              <ValidationRecordsTableHeaderCell>{`Verification Time`}</ValidationRecordsTableHeaderCell>
            </TableRow>
          </TableHeader>
          <TableBody className={isRefreshing || isPaging ? "opacity-60 transition-opacity duration-200" : ""}>
            {isLoading && !isPaging ? (
              <>{skeletonRows}</>
            ) : claims?.length > 0 ? (
              claims?.map((claim) => <BitcoinMirroringRecordRow key={claim.block_number} claim={claim} onViewError={handleViewError} />)
            ) : (
              <TableRow>
      ...
      <div className="mt-6">
        <Pagination currentPage={currentPage} totalPages={effectiveTotalPages} onPageChange={onPageChange} disabled={isLoading || isRefreshing || isPaging} />
      </div>
    </section>
  );
}
