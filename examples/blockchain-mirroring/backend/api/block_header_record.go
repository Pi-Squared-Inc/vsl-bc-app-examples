...
func RegisterBlockHeaderAPI(app *models.App) {
	...
	app.API.Get("/block_header_records", func(c fiber.Ctx) error {
		...
		detailQuery := app.DB.Model(&models.BlockHeaderRecord{}).
			Select("block_number, claim_id, chain, verification_time, error, created_at, claim, verification_context").
	...
	app.API.Get("/block_header_records/:claim_id/verification_context", func(c fiber.Ctx) error {
		claimID := c.Params("claim_id")
		if claimID == "" {
			return c.Status(400).SendString("claim_id is required")
		}
		...
		if dbHeaderRecord.VerificationContext == nil {
			return c.Status(404).SendString("verification context not found")
		}

		var verificationContextData any
		err = json.Unmarshal([]byte(*dbHeaderRecord.VerificationContext), &verificationContextData)
		if err != nil {
			return c.Status(400).SendString("invalid verification context")
		}

		return c.JSON(verificationContextData)
	})
