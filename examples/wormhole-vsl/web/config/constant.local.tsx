...
const anvil = {
  ...
  rpcUrls: {
    default: {
      http: [
        `${process.env.NEXT_PUBLIC_SOURCE_API ?? "http://localhost:8545"}`,
      ],
      webSocket: [
        `${process.env.NEXT_PUBLIC_SOURCE_WS_URL ?? "ws://localhost:8545"}`,
      ],
    },
  },
  name: "Anvil-1",
...
const anvil1 = {
  ...
  rpcUrls: {
    default: {
      http: [`${process.env.NEXT_PUBLIC_DEST_API ?? "http://localhost:8546"}`],
      webSocket: [
        `${process.env.NEXT_PUBLIC_DEST_WS_URL ?? "ws://localhost:8546"}`,
      ],
    },
  },
  name: "Anvil-1",
...
export const networks: AppKitNetwork[] = [anvil, anvil1];
